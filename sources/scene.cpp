...
Scene::init(std::string_view filepath)
{
  ...
  LOG_INFO(
    std::format("{} scene is loaded | Processing ...", filepath).c_str());
  ...
  LOG_INFO(
    std::format("Scene creation of {} file is finished", filepath).c_str());
...
void
Scene::calculate_bounds(Mesh& new_mesh, size_t initial_vertex)
{
...
void
Scene::assign_transforms(std::unordered_map<std::string, MeshNode>& _nodes,
                         ...
                         fastgltf::Asset&                           gltf)
{
  for (fastgltf::Node& node : gltf.nodes) {
    ...
    if (node.meshIndex.has_value()) {
      ...
      new_node.mesh         = &_meshes[mesh_name];
    }
